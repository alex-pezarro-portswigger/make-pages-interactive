...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    <span class="cf-header-hint">F · P · H · S · G · T · ↵ · ? · Esc</span>',
      ...
      '      <button id="cf-elem-toggle" class="cf-btn" title="Select element (S)">🎯 select element <span class="cf-kbd-hint">S</span></button>',
      ...
      '      <button id="cf-submit" class="cf-btn-primary" title="Submit pending (Enter)" disabled>submit batch <span class="cf-kbd-hint">↵</span></button>',
      '    </div>',
  ...
  function bindEvents() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        ...
        case "g": case "G":
          e.preventDefault(); openGeneralEditor(); break;
        case "Enter":
          if (
            $("cf-panel").classList.contains("cf-open") &&
            !$("cf-editor").classList.contains("cf-visible") &&
            pending.length > 0
          ) {
            e.preventDefault();
            submitBatch();
        ...
        case "?":
          e.preventDefault();
          showToast("F: feedback · P: pending · H: history · S: select element · G: general · T: tour · Enter: submit pending · R: reload · ←/→: tour nav · Esc: close", 7500);
          break;
      }
    });
  }

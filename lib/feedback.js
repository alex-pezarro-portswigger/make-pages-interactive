...
(function () {
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    <span class="cf-header-hint">F · P · H · S · T · ? · Esc</span>',
  ...
  function closeEditor() {
    const editor = $("cf-editor");
    // Blur before hiding: otherwise the textarea keeps focus, keystrokes route
    // to it (silently accumulating, then surfacing in the next editor), and
    // isTypingTarget() blocks the global shortcut handler.
    $("cf-editor-text").blur();
  ...
  function bindEvents() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        ...
        case "?":
          e.preventDefault();
          showToast("F: feedback · P: pending · H: history · S: select element · T: tour · R: reload when changes ready · ←/→: tour nav · Esc: close", 6500);
          break;
      }
    });
  }
